...
FROM    VP60_Spwy..return_document                   rd WITH (NOLOCK)
 
JOIN    VP60_Spwy..return_item                       ri WITH (NOLOCK)
ON      ri.business_unit_id               = rd.business_unit_id
AND     ri.return_id                      = rd.return_id
 
JOIN    VP60_Spwy..return_supplier_item              rsi WITH (NOLOCK)
...
JOIN    VP60_Spwy..unit_of_measure                   uompkg WITH (NOLOCK)
ON      uompkg.unit_of_measure_id         = rsi.packaged_in_uom_id
 
LEFT OUTER JOIN VP60_Spwy..unit_of_measure           uomprc WITH (NOLOCK)
ON      uomprc.unit_of_measure_id         = rsi.priced_in_uom_id
 
LEFT OUTER JOIN VP60_Spwy..item_uom_conversion       uomcvpkg WITH (NOLOCK)
ON      uomcvpkg.unit_of_measure_class_id = uompkg.unit_of_measure_class_id
AND     uomcvpkg.item_Id                  = rsi.inventory_item_id
 
LEFT OUTER JOIN VP60_Spwy..item_uom_conversion       uomcvprc WITH (NOLOCK)
...
AND     NOT EXISTS( SELECT 1                                -- not to include returns having non-saleable quantity
                      FROM VP60_Spwy..received_document  AS recd
                      JOIN VP60_Spwy..purchase_order     AS po
...
JOIN    VP60_Spwy..return_item_component_list        ricl WITH (NOLOCK)
