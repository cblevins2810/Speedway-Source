...
DECLARE @cost_level TABLE (supplier_id INT, merch_cost_level_id INT, cost_level_name nvarchar(100))

-- Added for support of incremental extract
DECLARE	@last_max_id	INT
-- Set this to the max Item Id from the last extract
SET		@last_max_id	= 0 --2396774
...
SET @Modulus = 1
SET @Remainder = 0
SET @SupplierXRef = 'BERNALC'
--SET @SupplierXRef = 'ABBEVRGE'
...
UNION
SELECT DISTINCT s.supplier_id, l.merch_cost_level_id, mcl.name
...
UNION
SELECT DISTINCT s.supplier_id, l.merch_cost_level_id, 'F_' + mcl.name
FROM   Merch_bu_spi_cost_list AS l
JOIN   supplier AS s
ON     l.supplier_id = s.supplier_id
JOIN   business_unit AS bu
