/****** Object:  StoredProcedure [dbo].[ape_insert_execution_object]    Script Date: 10/6/2018 3:13:34 PM ******/
...
/* 
   This is a custom change made to bypass the ape depletion job during a fist time/one time load of items.
   If the moniker for the depletion job is attempted to be queued, the proc will return a prior completed
   depletion job.
   
*/

IF @moniker = 'applications.MerchandiseManagement.ape.jobs.MM_Process_Depletion'
BEGIN
select TOP 1
...
child_count,
in_process_status
FROM APE_Execution_Queue
WHERE execution_moniker = 'applications.MerchandiseManagement.ape.jobs.MM_Process_Depletion'
