exec sp_executesql N'SELECT  button.button_id AS button_id, button.button_name AS button_name, button.text_color_id AS text_color_id, 
...
FROM (SELECT * FROM  pos_button_template t WHERE  client_id in (@cdm_client_id, 0)) t
JOIN (SELECT * FROM  (select d.color_id, d.frame_group_type_id, d.frame_resolution_type_code, d.global_flag, d.height, d.left_position, d.pos_frame_template_id, d.top_position, d.width, d.client_id, d.data_guid, coalesce(m.description, d.description) as description, d.last_modified_timestamp, d.last_modified_user_id from pos_frame_template as d 
   left join pos_frame_template_lang as m
     on m.translated_lang_id = @current_language_id
  and m.pos_frame_template_id = d.pos_frame_template_id) ft WHERE  client_id in (@cdm_client_id, 0)) ft
