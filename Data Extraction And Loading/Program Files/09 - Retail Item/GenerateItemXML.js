...
// Jan 2019: Made changes to accomodate additional 5 columns for Item Group for a total of 10
...
{ 
	...
	{ 
		...
		if (bFirstTime) 
		{
			...
			// Number of array elements = 103 (original file) and = 113 (file with item groups)
		...
		if (priorItemExternalId != vInputLine[0]) 
		{
			...
			var retailPackList = [];
			var attributeList = [];
			// Added to accomodate the additional 10 columns for Item Group
			var itemGroupList = [];
		...
		if (bFristLineOfItem)
		{
			...
			{
				// Added to account for the additional 10 columns for Item Group
				for (i = 103; i <= 112; i+=1)   
				{
					if (vInputLine[i] != '')
					{
