...
ELSE
   SELECT COUNT(DISTINCT itemXrefId), 'Items Verified' FROM bc_extract_repair_manufacturer_and_strategy
...
IF (SELECT COUNT(*)
	FROM bc_extract_repair_manufacturer_and_strategy
	WHERE retail_modified_item_id IS NULL) > 0

BEGIN
	
	SELECT 'Missing RMI', * FROM bc_extract_repair_manufacturer_and_strategy
	WHERE retail_modified_item_id IS NULL
...
IF (SELECT COUNT(*)
	FROM bc_extract_repair_manufacturer_and_strategy
	WHERE merch_group_id IS NULL) > 0
...
IF (SELECT COUNT(*)
	FROM bc_extract_repair_manufacturer_and_strategy
	WHERE merch_group_member_id IS NULL) > 0

BEGIN
	
	SELECT 'Missing Merch Group Member', *
	FROM bc_extract_repair_manufacturer_and_strategy
	WHERE merch_group_member_id IS NULL
...
IF (SELECT COUNT(*)
	FROM bc_extract_repair_manufacturer_and_strategy
	WHERE default_merch_level_id IS NULL) > 0
